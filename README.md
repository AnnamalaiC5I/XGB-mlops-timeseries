...
```bash
conda create -n xgboost_mlops python=3.9
conda activate xgboost_mlops
```
...
```
This command will create your personal repository under `/Repos/<username>/xgboost_mlops`.
...
- To trigger the release pipeline, get the current version from the `xgboost_mlops/__init__.py` file and tag the current code version:
```
